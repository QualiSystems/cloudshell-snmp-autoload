...
class SnmpV2MibData(object):
    ...
    def get_system_name(self):
        if not self._sys_name:
            self._sys_name = self._snmp_handler.get_property(snmpv_v2_constants.SYS_NAME)
        return self._sys_name

    def get_system_location(self):
        if not self._sys_location:
            self._sys_location = self._snmp_handler.get_property(snmpv_v2_constants.SYS_LOCATION)
        return self._sys_location

    def get_system_contact(self):
        if not self._sys_contact:
            self._sys_contact = self._snmp_handler.get_property(snmpv_v2_constants.SYS_CONTACT)
